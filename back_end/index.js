...
app.post("/hostCreateEvent", async (req, res) => {
  try {
    const { hostId, title, date, description} = req.body;
    if (!(hostId && title && date && description)) {
      return res.status(400).send("All input is required");
    }
...
app.post("/userEvents",async(req,res)=>{
  try {
    ...
    if (userId){
      const user = await Host.findOne({hostId});

      const userEvents= host.events;
...
app.post("/startEvent",async(req,res)=>{
  try {
    ...
    if (event.hostId= hostId){
      ...
      const update={ $set:{streamId:streamId}}
